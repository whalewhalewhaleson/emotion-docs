---
title: Choosing Your Surface
description: Bot chat, Mini App, or web dashboard — match the surface to the cognitive mode of the task.
publish: true
---

Pick by **cognitive mode**, not by what is easiest to build.

| Surface       | Feels like              | Best for                                               |
| ------------- | ----------------------- | ------------------------------------------------------ |
| **Bot**       | Walk past, jot, move on | Repeatable 2-minute capture, status checks, broadcasts |
| **Mini App**  | Mobile, but two-handed  | Browsing, editing, layouts that do not fit chat        |
| **Dashboard** | Sit down and compare    | Oversight, reports, admin, exports                     |

All three can read the same database. The split is interface, not infrastructure.

## Rules that held up

- **Residency beats reachability.** A tool people can open loses to the app already open. Telegram won because the team lived there.
- **Graduate when commands sprawl.** Past 3-5 visible bot commands, move browsing, history, and editing into a Mini App.
- **Use dashboards for comparison.** Leadership needs a big screen, filters, export, and session state.
- **Keep groups write-only.** In group chats, the bot broadcasts. Commands and sensitive flows stay in DMs.

Related: [[wiki/telegram-bots/authentication-and-access|Authentication & Access]]
